...
def investigation_prompt(seed_type: str, seed_value: str) -> str:
    ...
    if seed_type == "url":
        ...
        return (
            ...
            f"STEP 4: companies_house_lookup(\"{seed_value}\") (UK companies; best-\n"
            "        effort — skip cleanly if available=false / no key). Graph each\n"
            "        officer and PSC as a `person` node and link it to the company\n"
            "        (relation `officer_of` for directors, `significant_control_of`\n"
            "        for PSC, source=\"companies_house\"). PSC is registry-declared\n"
            "        control — still ESTIMATED ownership, not authoritative UBO.\n"
            f"STEP 4b: edgar_lookup(\"{seed_value}\") (US-listed issuers; best-\n"
            "        effort — skip if found=false). Add CIK / tickers / SIC and any\n"
            "        former names (as aliases) onto the company node.\n"
            f"STEP 4c: recherche_entreprises_lookup(\"{seed_value}\") (French\n"
            "        companies; best-effort — skip if found=false). Add SIREN; graph\n"
            "        each natural-person dirigeant as a `person` node (officer_of\n"
            "        edge); a dirigeant of kind=company → graph as a company.\n"
            f"STEP 5: sanctions_screen(\"{seed_value}\") — and screen EACH parent /\n"
            "        subsidiary company AND each officer/PSC `person` you graphed.\n"
            "        On a hit: tag that node `sanctioned`, record the programme(s) +\n"
            "        list + ref. A hit is a CANDIDATE match for human review (name\n"
